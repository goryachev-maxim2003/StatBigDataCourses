...
- [Сравнение средних](#сравнение-средних)
  ...
  - [Доверительный интервал через t-критерий](#доверительный-интервал-через-t-критерий)
  - [Проверка распределения на нормальность, QQ-Plot](#проверка-распределения-на-нормальность-qq-plot)
...
box-plot - график, где линия по середине - медиана, где прямоугольник - между 1 и 3 квартилем и 2 линии (ещё 1,5 межквартальных размаха (IQR) (разность значений 1 и 3 квартиля) от краёв прямоугольника)\
...
нарисовав box-plot для каждой из групп можно сравнить распределения по изменчивости. Здесь значения перемешаны
...
!в следующих разделах описано t-распределение, через которые лучше строить доверительный интервал, особенно, когда число элементов выборки меньше 30.
...
примечание: t-распределение иногда используется для не нормально распределённых показателей на практике. Но нужно быть осторожным с возможным возникновением нежелательных спецэффектов
...
- гомогенность дисперсий (дисперсии сравниваемых величин должны быть одинаковыми). Проверить можно при помощи критерия левина и критерия фишера
- если объём выборки < 30, то важно, чтобы распределение было нормальным


## Доверительный интервал через t-критерий

считаем также, как и по нормальному распределению, только вместо 2 сигм для 95% интервала ищем $t_{кр}$ для нужного нам уровня значимости (например, 5%) для t-распределения с определённым количеством df (степеней свободы) и

## Проверка распределения на нормальность, QQ-Plot

самый примитивный способ построить гистограмму частот и визуально сравнить с графиком нормального распределения

![](img/2025-01-15-17-42-02.png)

**QQ-plot** - показывает, как выборочные значения отклоняются от предсказанных (распределённых нормально)

![](img/2025-01-15-17-45-25.png)

ось x - квантили в нормальном распределении

ось y - квантили в выборке

например, если мы делим выборку на 15 частей, то у нас 14 квантилей

аналогично делим на квантили нормальное распределение

к примеру если с левого края значения сильно выше, то квантиль сместиться ниже чем нужно,

справа наоборот,  если с левого края значения сильно выше, то квантиль сместиться выше чем нужно

когда мало значений то график qq-plot удобен

**Влияние выбросов**

пару выбросов могут сильно повлиять на t-критерий

если распределение уходит от нормального, есть критерий Mann - Whitney

t-тест чувствителен к выбросам

## Однофакторный дисперсионный анализ

H0 - все средние равны

H1 - хотя бы 2 средних значений отличаются

пример: 3 группы

![](img/2025-01-18-12-51-37.png)


SST (sum of squares total) - общая сумма квадратов (какова изменчивость данных без учёта разделения на группы)

$$
\overline{\overline{x}}  = \frac{\sum{x_{всех\_групп}}}{n_{всех\_групп}}
$$

![](img/2025-01-18-12-54-02.png)

Основная идея дисперсионного анализа: SSобщая = SSфакторная + SSостаточная

SSt - общая сумма квадратов

df = N-1

![](img/2025-01-18-14-32-07.png)

![](img/2025-01-18-12-54-58.png)

источники SST:
- SSB (sum of squares between groups) - межгрупповая сумма квадратов (факторная)
- SSW (sum of squares within groups) - внутригрупповая сумма (остаточная)

Как считать SSW факторную